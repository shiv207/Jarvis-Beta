...
def MainTaskExecution(Query):
    ...
    try:
        if "open" in ReturnData:
            ...
            from Nasa import get_nasa_image
            st.set_page_config(page_title='NASA Image of the Day', page_icon=':rocket:')
            st.title('NASA Image of the Day')
            date = st.date_input('Select a date', max_value=dt.date.today(), value=dt.date.today())
            if st.button('Get Image'):
                with st.spinner('Fetching image...'):
                    title, explanation, image_path = get_nasa_image(str(date))
                    st.write(title)
